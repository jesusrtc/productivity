...
from lab.agent_instructions import NOTEBOOK_AGENT_SECTION, NOTEBOOK_SECTION_MARKER
...
def _ensure_notebook_section(agents_md: Path, *, actions: list[str],
                             root: Path, dry_run: bool) -> None:
    """Append the live notebook execution contract to AGENTS.md once."""
    if not agents_md.is_file():
        return
    text = agents_md.read_text(encoding="utf-8")
    if NOTEBOOK_SECTION_MARKER in text:
        return
    if not dry_run:
        sep = "\n" if text.endswith("\n") else "\n\n"
        agents_md.write_text(
            text + sep + NOTEBOOK_AGENT_SECTION + "\n",
            encoding="utf-8",
        )
    actions.append(f"add Lab notebooks section to {_rel(agents_md, root)}")
...
def sync_all(root: Path, *, dry_run: bool = False) -> dict:
    ...
    _migrate_instructions(root, create_stub=True,
                          actions=actions, root=root, dry_run=dry_run)
    _ensure_notebook_section(root / "AGENTS.md",
                             actions=actions, root=root, dry_run=dry_run)
...
def sync_notebook_instructions(root: Path, *, dry_run: bool = False) -> dict:
    ...
    _migrate_instructions(
        root, create_stub=True, actions=actions, root=root, dry_run=dry_run,
    )
    ...
    if projects_root.is_dir():
        for pdir in sorted(projects_root.iterdir()):
            ...
            _ensure_notebook_section(
                pdir / "AGENTS.md", actions=actions, root=root, dry_run=dry_run,
            )
    return {"actions": actions, "dry_run": dry_run}
