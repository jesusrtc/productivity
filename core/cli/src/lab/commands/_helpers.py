...
Centralizes the ``--workspace``/PWD resolution pattern so subcommand files
(``pr.py``, ``artifact.py``, ``search.py``, ``workspace.py``, ``task.py``) share
...
def require_valid_id(workspace_id: str) -> str:
    """Validate a workspace id and surface ModelError as ClickException."""
    try:
...
def resolve_workspace_id(explicit: str | None) -> str:
    """Return the workspace id the user is operating on.

    If ``explicit`` is given, validate and return it. Otherwise walk up from
    PWD looking for a workspace folder. Raises ClickException if neither works.
    """
    if explicit:
