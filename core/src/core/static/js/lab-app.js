  ...
  function renderRepoTabs() {
    ...
    if (isAssistant) {
      ...
      for (const [index, layout] of (window.AssistantView?.taskLayouts || []).entries()) {
        html += `<button class="repo-tab${assistantSection === layout.id ? ' active' : ''}" data-assistant-section="${layout.id}" onclick="AssistantView.setSection('${layout.id}')" title="${escAttr(layout.name)}" style="font-weight:600">&#x2726; Tasks ${index + 1}</button>`;
      }
