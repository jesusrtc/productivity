...
export async function render(parent, { params }) {
  ...
  const input = h("input", {
    type: "text",
    placeholder: "Search workspaces, tasks, and docs…",
    value: initialQuery,
  ...
  function renderResults(r) {
    const total = r.workspaces.length + r.tasks.length + r.docs.length;
    status.textContent = `${total} result${total === 1 ? "" : "s"} for “${r.query}”`;
    const sections = [];
    if (r.workspaces.length) {
      sections.push(h("section", null,
        h("h3", null, `Workspaces (${r.workspaces.length})`),
        h("ul", { style: "list-style:none; padding:0;" },
          ...r.workspaces.map((p) => h("li", { style: "margin:6px 0; font-size:13px;" },
            h("a", { href: `#/w/${p.id}` }, p.id),
      ...
      sections.push(h("section", null,
        ...
        h("ul", { style: "list-style:none; padding:0;" },
          ...r.tasks.map((t) => h("li", { style: "margin:6px 0; font-size:13px;" },
            h("a", { href: `#/w/${t.workspace_id}` }, `${t.workspace_id}#${t.task_id}`),
