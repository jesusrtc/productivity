def test_index_exposes_framework_and_active_vault_roots_separately(
    ...
    assert f'window.LAB_VAULT_ROOT = "{monorepo.resolve()}"' in r.text
...
def test_productivity_is_default_and_vault_selector_is_retired(client) -> None:
    r = client.get("/")

    assert r.status_code == 200
    assert '<body class="self-active">' in r.text
    assert 'id="vaultSelect"' not in r.text
...
def test_frontend_declares_cross_vault_navigation_surfaces() -> None:
    from pathlib import Path

    root = Path(__file__).resolve().parents[2]
    script = (root / "core/src/core/static/js/lab-app.js").read_text()

    assert "function fetchVaultCatalog" in script
    assert "function goToVault(vaultId" in script
    ...
    assert "function vaultSaveAppearance" in script
    assert "function workspaceSaveDisplayName" in script
