...
def test_independent_record_tree_browser(client, monkeypatch, tmp_path, monorepo):
    from lab import assistant as db, assistant_records as records, assistant_migration as migration
    ...
    paths = ['tasks/'+task.name, 'tasks/'+child.name, 'notes/'+thread.name]
    details = {path:client.get('/api/assistant/'+ ('note' if path.startswith('notes/') else 'task'),params={'path':path}).json() for path in paths}
    fixtures = {'index':client.get('/api/assistant').json(), 'details':details, 'paths':paths, 'project':project.stem}
    checks = r'''
...
window.fetch=async (url,options={})=>{
 ...
 return {ok:!!record,json:async()=>record||{detail:'Missing'}};
};
(async()=>{
 ...
 assert(FIX.details[FIX.paths[1]].metadata.project===FIX.project,'project is editable');
 assert(FIX.details[FIX.paths[1]].metadata.workspace==='demo','project change keeps workspace');
 nav.querySelector(`[data-record-path="${FIX.paths[2]}"]`).click();
 await until(()=>host().textContent.includes('Keep nested notes.'));
 assert(!field('priority')&&field('workspace'),'note properties are independent');
 ...
 await until(()=>field('priority'));
 assert(field('project').value==='','child change did not change parent');
 ...
 assert(document.querySelectorAll('[data-assistant-note]').length===1,'plain notes list excludes task files');
