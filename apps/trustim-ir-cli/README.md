...
### Via Phoenix (recommended)

The `ir` CLI is distributed via GULL. On macOS, it installs automatically via Phoenix (every ~3 hours), or you can trigger it manually:

```bash
sudo phoenix --gull
```

After installation, `ir` is available at `/usr/local/linkedin/bin/ir`.

### As a dependency in another MP

Other multiproducts can depend on `trustim-ir-cli` as a runtime dependency. In your MP's `build.gradle`:

```groovy
dependencies {
    python spec.product.'trustim-ir-cli'.'trustim-ir-cli'
}
```

This makes the `ir` command and the `trustimircli` Python package available in your MP's environment.
