...
`darwin-backups`, `trustim-investigation`, `trustim-ir-cli`, `~/workspaces/*`)
...
- [x] `content/workspaces/<id>/workspace.json + tasks.json + docs/ + notes/ + assets/`
...
- [ ] All 14 workspaces under `~/workspaces/*` migrated into `content/workspaces/*`,
  each with a proper `workspace.json` + `tasks.json` converted from the old
...
- [x] Framework-owned code no longer lives under `apps/`; that folder is reserved for vault-owned apps.
- [ ] Personal/client CLIs such as old `darwin-backups` or `trustim-ir-cli` should live in a vault repo or a standalone repo, not in the Lab framework checkout.
...
- [ ] `lab workspace add <name> <mp>` creates a git worktree at
  `content/workspaces/<name>/<prefix>-<objective>/` on branch `jcortes/<objective>`
- [ ] `lab workspace remove <name> <mp>` cleans up the worktree
...
- [x] `lab workspace new|ls|status|set|archive|rm`
- [x] `lab task new|ls|show|set|done|reopen|block|unblock`
- [x] `lab index rebuild|show`
- [x] `lab start|stop|open`
- [ ] `lab workspace add|remove` (worktrees — §C)
- [ ] `lab mp ls|prefix` (MP prefix management — §C)
- [ ] `lab search "keyword"` (full-text across workspaces/tasks/docs)
- [ ] `lab pr add <url> [--workspace <id>] [--mp <mp>]`
- [ ] `lab artifact add <url> [--workspace <id>] [--type ...]`
...
- [x] `GET /api/index | /api/workspaces | /api/workspaces/{id} | /api/workspaces/{id}/tasks | /api/tasks | /api/tasks/due | /api/markdown | /api/workspaces/{id}/docs | /api/workspaces/{id}/file`
- [x] `POST /api/workspaces | /api/tasks | /api/tasks/{p}/{t}/status | /api/tasks/{p}/{t}/update`
...
- [ ] `GET /api/diff?workspace=<id>&mp=<mp>` (diff view of worktrees vs master — folds gdiff in)
- [ ] `GET /api/commits?workspace=<id>&mp=<mp>` (commit list per worktree)

### F. Frontend — SPA on `:3333/`

- [x] Dashboard: workspace grid + due-soon strip + status filter
- [x] Workspace view: Tasks tab + Docs tab
- [x] Timeline: List (bucketed) + Gantt sub-views
- [x] Markdown viewer
- [x] WS live refresh
- [ ] **Replace `prompt()` dialogs with an inline form modal** (new workspace / new task)
- [ ] **Docs tab: render `.md` as links to markdown viewer, other files as links to `/api/workspaces/{id}/file`** (currently broken for non-md)
...
- [ ] Diff tab on workspace view (§E)
- [ ] Artifacts tab on workspace view (`lab artifact add` surface)
...
- [ ] `.claude/agents/intake-processor.md` ← copied from `apps/workspace/workspace.sh` agent block
...
- [x] Per-workspace `CLAUDE.md` auto-generated on `workspace new`
...
- [ ] Optional `PostToolUse` hook example in `.claude/settings.json` showing how to auto-log `git commit` SHAs into `workspace.json.prs[]`
...
- [ ] `make seed` creates 2–3 sample workspaces for demos / new installs (optional)
...
| Section | Status |
|---|---|
| A. Content migrated in | **0/21 items done**. Monorepo is empty. All content still in `~/src/productivity-old/` and `~/workspaces/`. |
...
| C. Repositories + worktrees | **0/6 done**. No `repositories/` folder, no `make pull-repos`, no `lab workspace add`. |
| D. CLI | **Core done; advanced missing.** Workspace/task/index/service commands ship. Worktree, search, PR, artifact, note, migrate commands missing. |
...
| H. Claude integration | **Partially done.** Root CLAUDE.md + per-workspace CLAUDE.md exist. No `~/.claude/rules/` link-in, no example hook. |
...
1. **Content migration** (§A) — copy cerebro/* into content/*, run a migration sweep on the 13 keeper workspaces under `~/workspaces/*`. After this step the user can actually use the monorepo with their real data instead of a blank dashboard.
2. **Frontend polish** (§F bullets) — inline form modal for new workspace/new task, fix the docs tab for non-md files, filter done tasks out of the due strip, wire WS auto-reconnect.
3. **Worktree commands** (§C + §D) — `lab workspace add/remove`, MP prefix config, `make pull-repos`. Enables coding work against lipy-davi / abuse-scoring-rules / abuse-short-term-action.
4. **Vault app migration** (§B) — keep personal/client tools such as old darwin-backups or trustim-ir-cli outside the framework checkout, either in a Lab vault's `apps/` folder or in standalone repos. (darwin-runner is retired — see `docs/DARWIN.md`.)
...
7. **Diff view** (§E + §F) — fold gdiff per-worktree diffs into `/api/diff` + a tab on the workspace view.
8. **Artifacts + PR tracking UI** (§D + §F) — CRUD forms for the already-existing `artifacts` / `prs` arrays in `workspace.json`.
