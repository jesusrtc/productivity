...
## User-requested stop: stable checkpoint for main (September 23, 2026)

The user requested stopping the latency goal once a stable checkpoint could be
pushed to main. No additional performance optimization is in progress. The last
search-capture prototype stayed in temporary files and was not integrated.
Checkpoint `db64d19` remained clean and independently validated with 2,110 passes,
one previously reproduced browser failure, and 11 slow tests excluded.

Main had advanced independently. The isolated `checkpoint/performance-main`
worktree combines fetched main `202ed64` with `db64d19`, preserving the user's
working directory and uncommitted edits. Integration keeps main's incremental
background Files snapshots, initial 202/retry contract, Git-tracked recent files,
non-following polling watcher, inline document behavior, cached scope containers,
and terminal transfer aliases/tombstones. The performance branch's generation
checks and markup reconciliation operate inside the existing scope containers.
Batched ownership resolution consults transfer aliases before stale indexes and
metadata; a combined regression checks tombstones, aliases, and fresh edits.
The explicit-recursion lifetime fix now applies to main's replacement walker.

Older fixtures were updated for the current interfaces without relaxing their
behavior assertions: pending notebook registry reads, discovery dependencies,
terminal access recording, and snapshot refresh completion. Materialized test
reads wait for the current complete scan; production snapshot tests continue to
use the ordinary nonblocking client. The Git rename fixture now has its own
repository so live vault log/index writes cannot contaminate its status.

The initial combined run recorded 1,928 passes and 59 failures. Seven of eight
selected checks also failed on unmodified `202ed64`; reports are retained under
`/tmp/lab-main-baseline-validation*`. Most failures were fixture mismatches.
A visual check reported two boundary pixels with a maximum channel difference
of 39; its unchanged assertions passed the focused rerun and final suite. No
visual threshold or latency budget was increased. All initial and focused
reports remain under `/tmp/lab-main-integration-check-1`,
`/tmp/lab-main-integration-focused*`, and `/tmp/lab-main-merge-lastchecks.log`.

The final combined source tree `3b9568a14d05d1fb3912e01b6d42634020e27181`
passed **1,988 core tests and 278 CLI tests: 2,266 passes, zero failures or skips**.
Eleven tests marked slow were excluded from the core run. All 101 changed Python
files parsed, all 20 changed JavaScript/module files passed `node --check`, and
`git diff --check` passed. Only these documentation/memory records were added
after that source-tree verification. Final reports, JUnit results and logs are
in `/tmp/lab-main-integration-final/`; cleanup found no owned processes or live
tmux sessions from the baseline, integration, or focused validation runs.

This is the requested stable checkpoint and stop, not proof that the full latency
objective was achieved. Earlier cold-start/concurrent/typing misses remain in
this document, and physical iTerm parity remains unverified. Resume optimization
