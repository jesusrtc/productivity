...
Companion to [vault-architecture.md](vault-architecture.md).

When a vault's agent configuration changes — a new default agent, an
added or removed supported agent, or a legacy vault being adopted — the
projected files in workspace trees need to follow. Neurona does not do this
itself. Paste the prompt below into your coding agent (Claude Code, Codex,
Copilot) from the vault root, let it work, and review the diff.
...
```text
I changed this vault's agent configuration. Bring the projected agent
files in line with it. Work from the vault root. This is guidance, not a
script — check what is actually on disk first, and show me anything surprising
instead of forcing it.

Where things live:

- `vault.json` at the vault root declares the supported agents, the
...
- Apply projections at the vault root and in every workspace under
  `workspaces/` that has a `workspace.json`.

What to do:

- For each projection enabled for a supported agent, make the target a
  relative symlink to the vault source. Use a generated pointer file only
...
- Remove projection targets that belong to agents no longer supported — but
  only when the target is a symlink into the vault sources or a file
...
- Never overwrite or delete a real file. If a projection target exists and is
  not a link into the vault or a marked generated file, stop and show me
  the conflict.
- Legacy layouts exist: older vaults have a real `AGENTS.md` at the root
  or inside a workspace, with `CLAUDE.md` linking to it. Move that content into
...
- Neurona needs no notification: it reads reality from disk, so open views and
  the Vault tab update on their own.
