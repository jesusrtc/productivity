...
#   2. $LAB_VAULT/.lab/state/server.port (written by the server on startup)
#   3. active vault in $LAB_HOME/vaults.toml or ~/.lab/vaults.toml
#   4. checkout-local runtime state / .lab-server.port (legacy)
#   5. client checkout LAB_PORT in .env
#   6. active vault [server].port in lab.toml
#   7. 3333
#
# Examples:
...
#   $(scripts/lab-url.sh --configured-port) # .env / vault configured port
#
# Any script, doc snippet, or Claude tool invocation that needs to talk to
# the lab server SHOULD route through this so both the vault setting and
# one-run `make start PORT=4444` overrides are discovered consistently.

set -e

LAB_VAULT="${LAB_VAULT:-${LAB_WORKSPACE:-}}"
...
registry = Path(os.environ["LAB_HOME"]).expanduser() / "vaults.toml"
legacy = registry.with_name("workspaces.toml")
if not registry.is_file() and legacy.is_file():
    registry = legacy
if registry.is_file():
    data = tomllib.loads(registry.read_text())
    active = data.get("active")
    for row in data.get("vaults", data.get("workspaces")) or []:
        if str(row.get("id")) == str(active):
            print(Path(str(row.get("path", ""))).expanduser())
            break
' 2>/dev/null || true
)"
configured_vault="${LAB_VAULT:-$active_vault}"
configured_port=""
if [[ -n "$configured_vault" && -f "$configured_vault/lab.toml" ]]; then
  configured_port="$(
    LAB_CONFIG_VAULT="$configured_vault" python3 -c '
import os, tomllib
from pathlib import Path
path = Path(os.environ["LAB_CONFIG_VAULT"]).expanduser() / "lab.toml"
try:
...
port="${LAB_PORT:-}"
if [[ -z "$port" && -n "${LAB_VAULT:-}" && -f "$LAB_VAULT/.lab/state/server.port" ]]; then
  port="$(tr -d '[:space:]' < "$LAB_VAULT/.lab/state/server.port")"
fi
if [[ -z "$port" ]]; then
  if [[ -n "$active_vault" && -f "$active_vault/.lab/state/server.port" ]]; then
    port="$(tr -d '[:space:]' < "$active_vault/.lab/state/server.port")"
  fi
fi
