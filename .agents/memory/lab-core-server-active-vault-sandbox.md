# Lab server needs out-of-sandbox start for external active vaults

When the active Lab vault is outside this repo, the core server writes
runtime logs/state under that vault's `.lab/state`. Starting it from the
