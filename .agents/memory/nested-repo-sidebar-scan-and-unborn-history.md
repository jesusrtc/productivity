...
- `/api/workspace-files` and `/api/workspace-mtime` share `_WORKSPACE_SCAN_SKIP_DIRS` and the same bounded-depth logic in `core/routes/diff.py`; keep them synchronized because sidebar contents and refresh detection must describe the same tree.
