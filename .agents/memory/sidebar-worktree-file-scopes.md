# Sidebar file scopes can follow worktrees

File-view settings may hold a browser-local parent directory. Only direct child
folders registered by Git as worktrees of the active repository are offered;
never treat every folder under that parent as a worktree. When a workspace is a
subdirectory of the main checkout, preserve its relative path in each linked
worktree. Discovery caches must vary by both parent folder and active base root.
Each sidebar surface remembers its own Root/worktree selection, and both
Recently updated and Files must use that same selected root.

Workspaces may be non-Git wrapper folders with registered repositories nested
inside them. Keep `main` file browsing rooted at the wrapper, but anchor Git
worktree discovery and repository history to the workspace's first registered
repository instead of the wrapper's nearest enclosing repository.

Worktree colors are browser-local, default to `#6e7681`, and appear as the
vertical scope rail around those sections. Keep notebook APIs pinned to the
active vault root even when ordinary file browsing uses another worktree.

A configured worktree parent may contain branch wrapper folders, with the
actual Git worktree nested below each wrapper. Offer the direct child wrapper
as the Files/Recently updated scope, but retain the nested checkout (including
any workspace-relative suffix) as the root for Git history. Direct-child Git
worktrees continue to use the matching workspace path for both roles.

Discovery must send both the visible folder scope and the registered repository
candidate. Prefer the visible scope only when it owns a `.git` marker, so a
direct Git workspace survives stale registered-worktree metadata without treating
a non-Git wrapper inside the vault monorepo as that monorepo's repository.
If the configured path is itself a direct-child linked worktree, normalize it to
its parent and include it in the choices. Exclude Git's primary checkout, not
the arbitrary checkout used to run `git worktree list`.
