# Vault workspaces use the Makefile dev-server standard

Every current and future vault workspace that runs a local dev server opts
...
access. Full contract: `docs/SERVERS.md`. The dashboard discovers any workspace
with this Makefile automatically and shows start/stop controls + live health.

Port assignments so far:

- `8001` — resume (`local` vault)
- `8002` — programming (`local` vault)
- `8003` — type-and-recall (`local` vault)
- `8005` — jaquemate (`productivity`/ssd vault)
- `8006` — remotion (`productivity`/ssd vault)
...
IPv4 health check (`127.0.0.1`) even though the server is up. If a workspace's
