...
direct URL over a same-origin one. Concretely, the per-workspace dev-server "Pop out"
(`openWorkspaceProxyTab`) now opens the **direct `http://host:port/...`** URL via the
