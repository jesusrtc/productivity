...
name: investigation
description: >-
  Run a trust/safety/metrics investigation. Use when the user pastes an alert,
  metric spike, Slack thread, or asks to investigate an anomaly. Produces a
  funnel-style investigation.md with charts per step, IOC isolation queries,
  and a Next Steps section. Each step in the main funnel must have a chart.
---
...
- `POST /api/nb/exec` on the lab server for matplotlib charts in a live `.ipynb` (see project CLAUDE.md → Darwin analyses)
